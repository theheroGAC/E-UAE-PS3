...
void init_screen(void *host_addr, u32 size, int video_mode)
{
	int result;
	...
	if (video_mode == 0) {
		video_mode = state.displayMode.resolution;
	}

	result = videoGetResolution(video_mode, &res);
	//failed to switch to the forced resolution
	if (result != 0 && video_mode != state.displayMode.resolution) {
		//use the default resolution
		video_mode = state.displayMode.resolution;
		videoGetResolution(video_mode, &res);
	}

	videoConfiguration vconfig;
	memset(&vconfig,0,sizeof(videoConfiguration));

	vconfig.resolution = video_mode;
