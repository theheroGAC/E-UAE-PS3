...
void restore_state (const char *filename)
{
	...
	write_log("restore failed. wrong header. %s\n", filename);
...
void save_state (const char *filename, const char *description)
{
    ...
    if (!f) {
	write_log("save: file open failed! %s \n", filename);
	return;
    }
#if 0
    ...
    write_log("save_cpu done  \n");
    free (dst);
    write_log("free CPU done h=%p dst=%p\n", header, dst);
#ifdef FPUEMU
    dst = save_fpu (&len,0 );
    write_log("save_fpu done \n");
    save_chunk (f, dst, len, "FPU ", 0);
    if (dst) 
    	free (dst);
    write_log("free FPU done \n");
#endif

    strcpy (name, "DSKx\0");
    for (i = 0; i < 4; i++) {
	write_log("saving disk...  %i \n", i);
	dst = save_disk (i, &len, 0);
	write_log("save_disk %i done  name=%s dst=%p \n", i, name, dst );
	if (dst) {
	    name[3] = i + '0';
	    save_chunk (f, dst, len, name, 0);
	    free (dst);
	    write_log("free DSK %i done \n", i);
	}
    }
    dst = save_floppy (&len, 0);
    write_log("save_floppy done \n");
